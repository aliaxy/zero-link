...
zero-link has completed Stage 4 redirect and cache implementation. Stages 1–4 are complete.

The repository intentionally has:

- Generated go-zero API and RPC services.
- API route contracts for health, readiness, administrator authentication, short-link management, and redirect.
- RPC methods for readiness, administrator authentication, short-link management, and short-link resolution.
...
Do not add analytics, management UI, new migrations, or additional business contracts unless the user
...
- `migrations/`: versioned SQL migrations. Stage 3 includes the initial administrator and short-link schema.
- `tests/httpyac/`: local HTTP smoke requests and example httpyac variables.
- `web/admin/`: reserved for the future embedded admin UI.

## Development Rules

- Preserve the current Stage 4 boundary unless the user explicitly asks for the next implementation stage.
...
- Do not add analytics APIs, management UI code, or additional migration SQL unless requested.
